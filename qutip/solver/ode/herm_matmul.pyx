...
cpdef Dense herm_matmul_dense(Dense left, Dense right,
    ...
    """
    _check_shape(left, right, out)
    cdef int N = <size_t> sqrt(right.shape[0])
    if right.shape[0] != N*N:
        raise ValueError(
            "invalid right matrix shape, must be a square operator-ket"
        )
    ...
    # upper triangular part.
    for dm_row in range(N):
        out.data[dm_row * (N+1) * row_stride] += scale * blas.zdotu(&N,
            &left.data[dm_row * (N+1) * row_stride], &col_stride,
            right.data, &one)
        for dm_col in range(dm_row+1, N):
            val = blas.zdotu(&N,
                &left.data[(dm_row * N + dm_col) * row_stride], &col_stride,
                right.data, &one)
            out[dm_row*N + dm_col] += scale * val
            out[dm_col*N + dm_row] += conj(scale * val)
    return out
...
cdef class _HermProdElement(_BaseElement):
    ...
    cdef Data matmul(_HermProdElement self, double t, Data state, Data out):
