...
cpdef CSR tidyup_csr(CSR matrix, double tol, bint inplace=True):
    ...
    cdef CSR out = matrix if (inplace and not matrix.immutable) else matrix.copy()
    nnz = 0
    out.row_index[0] = 0
...
cpdef Dense tidyup_dense(Dense matrix, double tol, bint inplace=True):
    cdef Dense out = matrix if (inplace and not matrix.immutable) else matrix.copy()
    cdef double complex value
    cdef size_t ptr
...
cpdef Dia tidyup_dia(Dia matrix, double tol, bint inplace=True):
    cdef Dia out = matrix if (inplace and not matrix.immutable) else matrix.copy()
