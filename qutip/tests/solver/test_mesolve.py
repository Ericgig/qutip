...
class TestMESolveStepFuncCoeff:
    ...
    @pytest.mark.parametrize('method',
                             all_ode_method, ids=all_ode_method)
    def test_py_coeff(self, method, fixture_random_seed):
        """
        Test for Python function as coefficient as step function coeff
        """
        rho0 = qutip.rand_ket(2, seed=fixture_random_seed)
    ...
    def test_array_cte_coeff(self, fixture_random_seed):
        """
    ...
    def test_array_t_coeff(self, fixture_random_seed):
        """
    ...
    def test_array_str_coeff(self, fixture_random_seed):
        """
    ...
    def test_array_str_py_coeff(self, fixture_random_seed):
        """
...
def test_krylovsolve_mixed_state(kdim, alg, fixture_random_seed):
    seeds = fixture_random_seed.spawn(2)
    H = qutip.rand_herm(20, density=.8, seed=seeds[0])
    rho0 = qutip.rand_dm(20, seed=seeds[1])
    e_op = qutip.num(20)
    e_op.dims = H.dims
