...
class TestIntegratorCte():
    ...
    @pytest.fixture(
        params=integrator_collection.list_keys('pairs', solver="sesolve"),
        ids=id_from_keys
    )
    ...
    @pytest.fixture(
        params=integrator_collection.list_keys('pairs', solver="mesolve"),
        ids=id_from_keys
    )
    ...
    @pytest.fixture(
        params=integrator_collection.list_keys('pairs', solver="mcsolve"),
        ids=id_from_keys
    )
    ...
    def test_mc_integration(self, mc_keys):
        ...
        for i in range(1, 21):
            ...
            while t < t_target:
                ...
                t, state = evol.step(t_target, step=True)
                assert t <= t_target
                assert t > t_old
                assert_allclose(self._analytical_se(t),
                                state.to_array()[0, 0], atol=2e-5)
                t_back = (t + t_old) / 2
                t_got, bstate = evol.step(t_back)
                assert t_back == t_got
                assert_allclose(self._analytical_se(t),
                                state.to_array()[0, 0], atol=2e-5)
                t, state = evol.step(t)
