...
def _make_plot_result(e_ops, solver="test_solver"):
    """Build a Result with 10 time steps for plotting tests."""
    N = 5
    res = Result(e_ops, fill_options(), solver=solver)
    for i in range(10):
...
class TestResultPlotExpect:
    ...
    def test_number_of_lines(self):
        a = qutip.destroy(5)
        result = _make_plot_result([a.dag() * a, a + a.dag()])
        fig, axes = result.plot_expect()
    ...
    def test_solver_name_as_title(self):
        a = qutip.destroy(5)
        result = _make_plot_result([a.dag() * a], solver="mesolve")
        fig, axes = result.plot_expect()
