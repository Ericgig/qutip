...
class TestFloquet:
    ...
    def testFloquetBasis(self, fixture_random_seed):
        N = 10
        a = destroy(N)
        H = num(N) + (a+a.dag()) * coefficient(lambda t: np.cos(t))
        T = 2 * np.pi
        floquet_basis = FloquetBasis(H, T)
        psi0 = rand_ket(N, seed=fixture_random_seed)
    ...
    def testFloquetUnitary(self, fixture_random_seed):
        N = 10
        a = destroy(N)
        H = num(N) + (a+a.dag()) * coefficient(lambda t: np.cos(t))
        T = 2 * np.pi
    ...
    def testFloquetMasterEquation1(self, fixture_random_seed):
        """
        ...
        psi0 = rand_ket(2, seed=fixture_random_seed)
    ...
    def testFloquetMasterEquation2(self, fixture_random_seed):
        """
    ...
    def testFloquetMasterEquation3(self, kmax, fixture_random_seed):
        """
        Test Floquet-Markov Master Equation for a two-level system
    ...
    def testFloquetMasterEquation_multiple_coupling(self, fixture_random_seed):
        """
        Test Floquet-Markov Master Equation for a two-level system
...
def test_fsesolve_fallback(fixture_random_seed):
