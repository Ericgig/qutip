...
class Test_file_data_store_file_data_read:
    ...
    def case(self, filename, kwargs, fixture_generator):
        data = 1 - 2*fixture_generator.random((_dimension, _dimension))
    ...
    def test_defaults(self, fixture_generator):
        return self.case(_random_file_name(), {}, fixture_generator)

    @pytest.mark.parametrize("type_", ["real", "complex"])
    @pytest.mark.parametrize("format_", ["decimal", "exp"])
    def test_type_format(self, type_, format_, fixture_generator):

        kwargs = {'numtype': type_, 'numformat': format_}
        return self.case(_random_file_name(), kwargs, fixture_generator)

    @pytest.mark.parametrize("separator", [",", ";", "\t", " ", " \t "],
                             ids=lambda x: "'" + x + "'")
    def test_separator_detection(self, separator, fixture_generator):
