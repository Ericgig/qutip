...
def test_unit_conversions(orig, target, fixture_generator):
    T = fixture_generator.random() * 100.0
...
@pytest.mark.parametrize('target', ["J", "eV", "meV", "GHz", "mK"])
def test_unit_conversions_loop(orig, middle, target, fixture_generator):
    T = fixture_generator.random() * 100.0
...
def test_unit_clebsch_delta_j(j1, j2, fixture_generator):
    """sum_m1 sum_m2 C(j1,j2,j3,m1,m2,m3) * C(j1,j2,j3',m1,m2,m3') =
    delta j3,j3' delta m3,m3'"""
    for _ in range(10):
        j3 = fixture_generator.choice(np.arange(abs(j1-j2), j1+j2+1))
        j3p = fixture_generator.choice(np.arange(abs(j1-j2), j1+j2+1))
        m3 = fixture_generator.choice(np.arange(-j3, j3+1))
        m3p = fixture_generator.choice(np.arange(-j3p, j3p+1))
...
@pytest.mark.parametrize('j1', [0.5, 1.0, 1.5, 2.0, 5, 7.5, 10, 12.5])
@pytest.mark.parametrize('j2', [0.5, 1.0, 1.5, 2.0, 5, 7.5, 10, 12.5])
def test_unit_clebsch_delta_m(j1, j2, fixture_generator):
    """sum_j3 sum_m3 C(j1,j2,j3,m1,m2,m3)*C(j1,j2,j3,m1',m2',m3) =
    delta m1,m1' delta m2,m2'"""
    for _ in range(10):
        m1 = fixture_generator.choice(np.arange(-j1, j1+1))
        m1p = fixture_generator.choice(np.arange(-j1, j1+1))
        m2 = fixture_generator.choice(np.arange(-j2, j2+1))
        m2p = fixture_generator.choice(np.arange(-j2, j2+1))
...
def test_zeta(fixture_generator):
    mpmath = pytest.importorskip("mpmath")
    N = 50
    s_list = fixture_generator.random(N) + 1.0001
    # could randomly fail if q is an integer <=0
    q_list = (fixture_generator.random(N) + 0.05) * 10
    for s, q in zip(s_list, q_list):
        assert utils.zeta(s, q) == pytest.approx(mpmath.zeta(s, q), rel=1e-12)

    q_list = (fixture_generator.random(N) * 0.9 - .95
              - fixture_generator.integers(10))
    for s, q in zip(s_list, q_list):
        assert utils.zeta(s, q) == pytest.approx(mpmath.zeta(s, q), rel=1e-12)

    s_list = fixture_generator.random(N) + 1.001
    q_list = ((fixture_generator.random(N) - 0.5) * 10
              + fixture_generator.standard_normal(N) * 10j)
    for s, q in zip(s_list, q_list):
        assert utils.zeta(s, q) == pytest.approx(mpmath.zeta(s, q), rel=1e-12)

    s_list = (
        1 + (fixture_generator.lognormal(size=N) + 1e-6)
        * np.exp((fixture_generator.random(N) * 0.8 - 0.4) * 1j * np.pi)
    )  # Real part > 1.000001
    q_list = ((fixture_generator.random(N) + 0.1) * 10
              + fixture_generator.standard_normal(N) * 10j)
    for s, q in zip(s_list, q_list):
        assert utils.zeta(s, q) == pytest.approx(mpmath.zeta(s, q), rel=1e-12)
...
class TestFitting:
    ...
    def generate_data(self, noisy, wavy, fixture_generator):
        """Generate test data."""
        ...
        if noisy:
            noise = fixture_generator.normal(0, 0.01, len(x))
            y += noise
        return x, y, fparams1, fparams2, noisy

    def test_fit(self, noisy, fixture_generator):
        x, y, fparams1, fparams2, noisy = self.generate_data(
            noisy, True, fixture_generator
        )
    ...
    def test_aaa(self, noisy, fixture_generator):
    ...
    def test_prony_methods(self, noisy, method, fixture_generator):
        x, y, _, _, noisy = self.generate_data(noisy, False, fixture_generator)
