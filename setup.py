...
def create_version_py_file(options):
    ...
    """
    filename = os.path.join(options['rootdir'], 'src', 'qutip', 'version.py')
    content = "\n".join([
...
def _create_int_type_file(options):
    ...
    path = os.path.join(options['rootdir'], 'src', 'qutip', 'core', 'data', 'src')
...
def create_extension_modules(options):
    ...
    if not options['openmp']:
        pyx_files -= set(root.glob('src/qutip/**/openmp/**/*.pyx'))
